...
├── pt.yml                    # Portuguese defaults
├── models/
│   ├── en.yml               # Model translations (EN)
│   └── pt.yml               # Model translations (PT)
├── views/
│   ├── en.yml               # View translations (EN)
│   └── pt.yml               # View translations (PT)
├── mailers/
│   ├── en.yml               # Mailer translations (EN)
│   └── pt.yml               # Mailer translations (PT)
└── components/
    ├── en.yml               # Component translations (EN)
    └── pt.yml               # Component translations (PT)
```
...
pt:
  activerecord:
    models:
      event: Evento
      event_vendor: Fornecedor de Evento
    attributes:
      event:
        name: Nome
      ...
      models:
        event:
          attributes:
            ...
            event_date:
              in_past: "não pode estar no passado"
```
...
pt:
  events:
    ...
    form:
      submit_create: Criar Evento
      submit_update: Atualizar Evento
    create:
...
class Event < ApplicationRecord
  ...
  # Event.model_name.human => "Event" or "Evento"
end
```
...
```yaml
# config/locales/components/pt.yml
pt:
  components:
    event_card:
      status:
...
I18n.l(Date.current)                    # "January 15, 2024" or "15 de janeiro de 2024"
I18n.l(Date.current, format: :short)    # "Jan 15" or "15/01"
I18n.l(Date.current, format: :long)     # "Wednesday, January 15, 2024" or "Quarta-feira, 15 de janeiro de 2024"
...
```ruby
# Number formatting
number_with_delimiter(1234567)          # "1,234,567" or "1.234.567"
number_to_currency(1234.50)             # "$1,234.50" or "R$ 1.234,50"

# With locale-specific formatting
number_to_currency(1234.50, locale: :pt)  # "R$ 1.234,50"
...
pt:
  ...
  notifications:
    unread:
      zero: Nenhuma notificação não lida
      one: Você tem 1 notificação não lida
      other: "Você tem %{count} notificações não lidas"
```

```ruby
# Usage
t("events.count", count: 0)   # "No events" or "Nenhum evento"
t("events.count", count: 1)   # "1 event" or "1 evento"
t("events.count", count: 5)   # "5 events" or "5 eventos"
```
